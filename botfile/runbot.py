...
def cmd_task_buttons(update, context):
    task_buttons = [[
        InlineKeyboardButton( '1.설명', callback_data=1 )
        , InlineKeyboardButton( '2.명언', callback_data=2 )
        , InlineKeyboardButton( '3.감정입력', callback_data=3 )
    ], [
        InlineKeyboardButton( '4.감정점수', callback_data=4 )
        , InlineKeyboardButton( '5.분석', callback_data=5 )
        , InlineKeyboardButton( '6.예측', callback_data=6 )
    ],[
...
def cb_button(update, context):
    ...
    if data == '7':
        ...
        context.bot.send_message(
            ...
            , text='다른작업을 선택하려면 /again을 입력하세요'.format( data )
        )
